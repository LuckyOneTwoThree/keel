...
- **文档层校验无条件执行** —— `check_project` 先跑 doc 级校验（subtype 枚举、ref 悬空、文件名/位置一致性），再按有无条目级 frontmatter 决定是否跑条目级校验。避免「还没立 REQ 的新项目」整类 doc 校验被静默跳过 —— 而「PRD 先写、REQ 事后补」恰是第 4 条地基约束鼓励的路径
- **「夹具绿 ≠ 生成器对」有专项守卫** —— `test_pm.py` 含一条 E2E：`pm new-req` → `pm finalize` 闭环后，整个项目必须仍能通过硬阻断校验。只测辅助函数无法发现「CLI 产出与校验规则不自洽」这类逃逸
